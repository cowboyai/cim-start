...
# we recommend <your machine on some IP>... the target being a different IP 
# and maybe your router the only other thing on this network during initial configuration
nix run github:nix-community/nixos-anywhere -- --flake .#vhost-dev root@192.168.1.100 -i ~/.ssh/id_cim_thecowboyai --debug
